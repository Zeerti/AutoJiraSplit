...
class JiraGeneratorWindow():
    ...
    def __init__(self, master, selenium):
        ...
        self.summary_static_text = Label(
            master, text="Input Sales Split Summary:")
        self.summary_static_text.grid(columnspan=4, sticky='W', row=1)

        self.summary_field = Entry(root, width=80,)
        ...
        self.clarify_field = Entry(root, width=80)
    ...
    def createCase(self):
        ...
        if(self.password == None or self.username == None or self.password == ' ' or self.username == ' '):
            ...
            self.firefox.inputDataToCase(self.summary_field.get(), self.clarify_field.get(), self.detailed_field.get("1.0", END))
